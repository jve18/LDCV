...
The domain of the problem depicted in Fig. \ref{fig:problem} is described in dimensional and non-dimensional quantities in Eqs. \ref{eq:domain_dim}, \ref{eq:domain_ndim}, respectively. A staggered grid is implemented, such that pressure nodes are placed in the center of the cells ($x_{1} = x_{1,m}$, $x_{2} = x_{2,m}$), $u_{1}$ nodes are placed at the mid-height of the west and east faces of the cells ($x_{1} = x_{1}$, $x_{2} = x_{2,m}$), and $u_{2}$ nodes are placed at the mid-width of the south and north faces of the cells ($x_{1} = x_{1,m}$, $x_{2} = x_{2}$), where the subscript ``$m$`` indicates ``mid-point''. The grid is uniform, as described in Eq. \ref{eq:grid_uniform}. An index ``$i$'' is used for the $x_{1}$ coordinate, and an index ``$j$'' is used for the $x_{2}$ coordinate. The coordinates of the the $u_{1}$, $u_{2}$, and pressure ($p$) nodes are tabulated in Table \ref{tab:grid_coord}, for clarity, and the grid is illustrated in Fig. \ref{fig:grid_hand}. There are ghost-nodes outside of the boundary, in both the $x_{1}$ and $x_{2}$ directions to enforce boundary conditions.
...
\begin{table}
  \begin{center}
    ...
    \begin{tabular}{ | c | c | c | }
      ...
      $u_{1}$ & $i\Delta{x}$ & $j\frac{\Delta{x}}{2}$ \\
      $u_{2}$ & $i\frac{\Delta{x}}{2}$ & $j\Delta{x}$ \\
      $p$ & $i\frac{\Delta{x}}{2}$ & $j\frac{\Delta{x}}{2}$ \\ \hline
    \end{tabular}
  \end{center}
